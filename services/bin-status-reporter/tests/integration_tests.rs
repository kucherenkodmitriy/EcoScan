//! Integration tests for the bin-status-reporter service
...
async fn test_update_bin_status() {
    // Skip if LOCALSTACK_ENABLED is not set to "true"
    if std::env::var("LOCALSTACK_ENABLED").unwrap_or_default() != "true" {
        return;
    }
